...
public class VectorMath : MonoBehaviour
{
    ...
    public TextMeshPro angleLabel; //***PUN made public
    [SerializeField, Tooltip("The angle arc")]
    private LineRenderer arc;

    [SerializeField, Tooltip("Dot Product Projection")]
    private PhotonLineRenderer dotProjection; //***PUN 
    [SerializeField, Tooltip("Dot Product Name Label")]
    public TextMeshPro dotNameLabel; //***PUN made public

    //***PUN   
    private PhotonView PV;
    [HideInInspector]
    public bool bCanPlaceAplusB, bCanPlaceAminusB, bCanPlaceAcrossB, bcanPlaceAdotB_Bcos, bCanPlaceAdotB_Acos, bCanRespawnB = false;
    [HideInInspector]
    public bool bVec2AnimationHappening = false;
    [HideInInspector]
    public bool bCanDisplayAngle = false;
    [HideInInspector]
    public bool bCanHideV2Labels = false;
    [HideInInspector]
    public bool bCanPlaceV2ComponentLabel = false;

    //[HideInInspector]
    //public bool bCanSpawnResVecCompLabel = false;
    ...
    void Start()
    {
        ...
        foreach (VectorControl v in vectors)
        {
    ...
    void Update()
    {
        ...
        if (GLOBALS.invertCross)
        {
            if (GLOBALS.didCross)
            {
                ...
                vectors[2].GetComponent<VectorControl>()._head.position = vectors[2].transform.position - crossProduct;
    ...
    public void PlaceVectorPoint(int v, bool isHead, Vector3 loc)  //v is vector index in list, loc is the position
    {
        ...
        if (isHead)
        {
            vectors[v].GetComponent<VectorControl>()._head.position = loc;
    ...
    public IEnumerator DoVectorAddition()
    {
        ...
        */
        ...
        bVec2AnimationHappening = true; // ***PUN before animating Vec 2        
        bCanHideV2Labels = true; //***PUN


        yield return StartCoroutine(vectors[1].AnimateVectorTranslation(vectors[0].GetComponent<VectorControl>()._head.position)); // animates position translation

        bCanRespawnB = true; //***PUN       

        vectors[2].transform.position = vectors[0].transform.position; // sets vector C position to vector A's tail 
        vectors[2].GetComponent<VectorControl>()._head.position = vectors[2].transform.position; // vector C head at the same position
        ...
        yield return StartCoroutine(vectors[2].AnimateVectorHead(vectors[1].GetComponent<VectorControl>()._head.position)); // moves head of vector 2 and passes parameters to define position of our Vector B
        vectors[2].SetEnabledLabels(false, false, true, false);

        bCanPlaceAplusB = true;// ***PUN A + B, after animation happens
        bCanPlaceV2ComponentLabel = true; //***PUN
        //bCanSpawnResVecCompLabel = true; //***PUN
    }
    ...
    public IEnumerator DoVectorSubtraction()
    {
        ...
        */
        ...
        Vector3 oldHeadPos = vectors[1].GetComponent<VectorControl>()._head.position; // get old head label position
        ...
        yield return StartCoroutine(vectors[1].AnimateVectorTranslation(vectors[0].GetComponent<VectorControl>()._head.position)); // animate the 3 vector translations to the head position

        bCanRespawnB = true; //***PUN after B has been renamed & Vec2 has been translated not only inverted  

        vectors[2].transform.position = vectors[0].transform.position; // transforms tail position to head position 
        vectors[2].GetComponent<VectorControl>()._head.position = vectors[2].transform.position; // get head component 
        vectors[2].gameObject.SetActive(true);  // activate head position object
        yield return StartCoroutine(vectors[2].AnimateVectorHead(vectors[1].GetComponent<VectorControl>()._head.position)); // animate the 3 vector head position 

        vectors[2].SetEnabledLabels(false, false, true, false);        

        bCanPlaceAminusB = true;// ***PUN A - B
        bCanPlaceV2ComponentLabel = true; //***PUN
        //bCanSpawnResVecCompLabel = true; //***PUN
    }
    ...
    public IEnumerator DoVectorCross()
    {
        ...
        */
        ...
        vectors[2].GetComponent<VectorControl>()._head.position = vectors[0].transform.position; // intializing vector C head position before animation
        ...
        bCanPlaceAcrossB = true;// ***PUN A x B  
        bCanPlaceV2ComponentLabel = true; //***PUN
        //bCanSpawnResVecCompLabel = true; //***PUN
    }
    #endregion
    ...
    private IEnumerator ProjectDotProduct(int n)
    {
        ...
        if (n == 0)
        {
            ...
            bcanPlaceAdotB_Bcos = true; //***PUN 
        }
            
        else
        {
            dotNameLabel.text = "|A|cos(" + theta.ToString("F0") + "°)";
            bCanPlaceAdotB_Acos = true; //***PUN 
        }           
    }
    #endregion
}
