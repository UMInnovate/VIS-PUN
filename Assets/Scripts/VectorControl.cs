...
public class VectorControl : MonoBehaviour
{
    ...
    [SerializeField] public TextMeshPro _nameLabel; //***PUN made public
    [SerializeField] public TextMeshPro _headLabel; //***PUN made these public
    [SerializeField] public TextMeshPro _tailLabel; //***PUN made these public
    [SerializeField] public TextMeshPro _componentLabel; //***PUN made public 
    ...
    private void InitLabels()
    {
        ...
        ////***PUN
        //rpcReceiverReference.InitLabels(gameObject.name); ???????commented out on rpcreceiver delete if not needed
    ...
    private void RotateLabelsTowardUser()
    {
        ...
        if (_componentLabel.enabled)
        {
            ...
            //***PUN trying this out
            // if component label is enabled && if that comp label is the resultant vec's comp label...
            if(gameObject.name == "SpaceVector Result" && runOnce)
            {
                bCanSpawnResVecCompLabel = true;
                runOnce = false; // makes sure the RPC is only called once, otherwise, 
                                 // because _componentLabel.enable keeps on returning true, 
                                 //the client will never stop spawning res vec comp labels!
            }
